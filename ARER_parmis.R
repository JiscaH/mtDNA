  ...
  for (y in 1:2) {  # high quality / medium quality SNP data
    for (m in 1:2) {  # without/with mtM
      ...
      for (x in seq_along(PM)) {
        if (PEDZ[z]=='HSg5' & PM[x] >0.9)  next   # TODO: later
...
EE <- c(1e-4, 5e-3)
PEDZ <- c('HSg5', 'swan')
PV <- c('2.5.6', '2.7.0', '2.7.0')

ARERT <- array(dim = c(length(PEDZ),2,3,length(PM),3),
               dimnames = list(PEDZ, c('goodQ', 'mediumQ'), c('2.5.6', '2.7.0', '2.7.0 +mt'),
                               PM, c('AR', 'ER', 'Time')))
for (v in 1:3) {
  for (z in seq_along(PEDZ)) {
    for (q in 1:2) {
      for (x in seq_along(PM)) {
        ...
        FileName <- paste0(FileName, ifelse(v==3, '_mt.RDS', '.RDS'))
#       cat(v, z, q, x, FileName, file.exists(FileName), '\n')
        ...
        ARERT[z,q,v,x,'Time'] <- ifelse(z==2, mean(EC$RunTime[EC$RunTime < 2000]),  # excl interrupted runs
                                        mean(EC$RunTime))/60   # time in minutes
      }
    }
  }
}
...
YLAB <- c("AR" = "False negative rate",
          ...
          "Time" = "Runtime (min)")
Yzero <- 1e-5
COL <- setNames(hcl.colors(n=3, palette="Zissou 1"), c('2.5.6', '2.7.0', '2.7.0 +mt'))
MaxTime <- c('swan' = 10, 'HSg5' = 20)


PZ <- 'HSg5' #  'swan'  #

par(mfcol=c(3, 2), mai=c(.3, .6, 0, 0), omi=c(.5,.4,.9,.1), xpd=F)
for (q in 1:2) {
  for (AE in c("AR", "ER", "Time")) {
    if (AE =="Time") Ylim <- c(0, MaxTime[[PZ]])
    if (AE == "AR")  Ylim <- c(Yzero, .05)
    if (AE == "ER")  Ylim <- c(Yzero, 0.03)
